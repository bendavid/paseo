...
export function buildEnsureScript(config: SshHostConfig, version: string): string {
  ...
  return [
    ...
    `fi`,
    `# 4. Launch the daemon (wrap in systemd-run --user --scope when available`,
    `#    to escape the session cgroup so the daemon survives SSH disconnect)`,
    `echo "PROGRESS:Launching the Paseo daemon on ${config.host}…" >&2`,
    `mkdir -p "${home}"`,
    `if command -v systemd-run >/dev/null 2>&1 && [ -n "$XDG_RUNTIME_DIR" ]; then`,
    `  # Enable linger so the user's systemd instance persists after SSH disconnect.`,
    `  # Without this, systemd-logind kills the user manager (and our scope) when`,
    `  # the last session closes — the daemon dies anyway, just via a different path.`,
